...
- 查看本地活动记录和当前运行状态
- 跟随系统、浅色和深色三种外观模式
- 菜单栏常驻，可直接查看状态、管理规则和修改设置

## 界面

Locus 是纯菜单栏应用，不会打开独立主窗口。点击菜单栏图标即可查看当前 Wi-Fi、输出设备、系统音量、静音状态、匹配规则和最近活动，也可以直接添加或编辑规则、调整偏好设置。

所有页面都在同一个紧凑 Popover 中切换。界面使用 SwiftUI 和 macOS 原生控件构建，低频的规则详情、活动记录和设置通过单层导航进入。
...
│   │   └── Views/MenuBar/       菜单栏首页、规则、设置与活动页面
